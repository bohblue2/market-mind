...
class FinanceNewsListPipeline:
    ...
    def process_item(self, item: Optional[NaverArticleItem], spider):
        ...
        if not isinstance(item, (NaverArticleItem, NaverArticleListFailedItem)):
            return item
        ...
        article = NaverArticleListOrm(
            ...
            article_published_at=kst.localize(
                datetime.strptime(
                    item["article_published_at"].strip(), "%Y.%m.%d %H:%M"
                )
            )
            if not item.get("category")
            else item["article_published_at"],
        )
...
class FinanceNewsContentPipeline:
    ...
    def process_item(self, item: NaverArticleContentItem, spider):
        if not isinstance(item, NaverArticleContentItem):
            return item
        response = item["response"]
        article = (
            self.sess.query(NaverArticleListOrm)
            .filter_by(
        ...
        exiting_record = (
            self.sess.query(NaverArticleContentOrm)
            .filter_by(
                article_id=response.meta["article_id"],
                media_id=response.meta["media_id"],
            )
        ...
        if exiting_record is not None:
            return DropItem(
                f"Article content already exists: {response.meta['article_id']}"
            )
        ...
        article_content = NaverArticleContentOrm(
            ...
            article_published_at=kst.localize(
                datetime.strptime(
                    item["article_published_at"].strip(), "%Y-%m-%d %H:%M:%S"
                )
            ),
            article_modified_at=kst.localize(
                datetime.strptime(
                    item["article_modified_at"].strip(), "%Y-%m-%d %H:%M:%S"
                )
            )
            if item.get("article_modified_at")
            else None,
        )
...
class ResearchMarketinfoListPipeline:
    ...
    async def process_item(self, item: Dict[str, Any], spider):
        if not isinstance(item, NaverResearchItem):
            return item
        research_report = NaverResearchReportOrm(
            ...
            target_company=item["report_item"].get("target_company", None),
            target_industry=item["report_item"].get("target_industry", None),
...
async def fetch_and_store_report(
    sess, report_orm: NaverResearchReportOrm, item: Dict[str, Any]
):
    ...
    try:
        ...
        report_file = NaverResearchReportFileOrm(
            report_id=report_orm.id, file_data=buffer
        )
