...
class NaverNewsArticleList(scrapy.Spider):
    ...
    async def parse(self, response: HtmlResponse) -> Any:
        ...
        for row in response.css("table.type5 tr"):
            ...
            article_data = self._extract_article_data(
                meta.get("ticker", "None"), row, processed_ids
            )
    ...
    def _fetch_tickers(self, limit: int = 5000) -> list:
        response = requests.get(
            f"http://127.0.0.1:8080/api/securities/code?tr_code=t8436&limit={limit}"
        )
        return [
            ticker["shcode"]
            for ticker in response.json()
            if not ticker["shcode"].endswith("K")
        ]
    ...
    def _get_target_url(cls, ticker: str, page: int = 1) -> str:
        return (
            f"https://finance.naver.com/item/news_news.naver?code={ticker}&page={page}"
        )
    ...
    def _extract_article_data(
        self, ticker: str, row, processed_ids: set
    ) -> Dict[str, Any] | NaverArticleErrorEnum:
        ...
        if (
            not article_id
            or not office_id
            or f"{office_id}{article_id}" in processed_ids
        ):
        ...
        if (
            article_published_at <= self.from_date
            or article_published_at >= self.to_date
        ):
    ...
    def _create_next_page_request(
        self, meta: Dict[str, Any], current_page: int
    ) -> scrapy.Request:
        return scrapy.Request(
    ...
    async def _handle_error(
        self, error_code: NaverArticleErrorEnum, response: HtmlResponse
